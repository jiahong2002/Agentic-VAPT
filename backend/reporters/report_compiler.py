...
<div class="page">
  ...
  <!-- ══ SAST FINDINGS ══ -->
  {% if scan.sast_findings %}
  <div class="section">
    <div class="section-heading">Static Analysis Findings (Source Code)</div>
    <table class="url-table" style="margin-bottom:0;">
      <thead>
        <tr>
          <th>Severity</th>
          <th>Tool</th>
      ...
      <tbody>
        {% for f in scan.sast_findings %}
        <tr>
          <td><span class="sev-badge sev-{{ f.severity.value }}">{{ f.severity.value }}</span></td>
          <td style="font-family:monospace;font-size:12px;">{{ f.tool }}</td>
          <td style="font-family:monospace;font-size:12px;word-break:break-all;">{{ f.file_path }}</td>
          <td style="font-family:monospace;font-size:12px;">{{ f.line_number or '—' }}</td>
          <td style="font-size:13px;">{{ f.message }}</td>
          <td><code style="font-size:11px;background:#f1f5f9;padding:2px 6px;border-radius:4px;">{{ f.code_snippet or '—' }}</code></td>
        </tr>
        {% endfor %}
      </tbody>
    </table>
